...
/* Only owned creature DBC names are redirected; the native reader owns all bytes. */
#define WXL_EXTENSION
#include "wxl/PluginApi.h"

#define PLUGIN_NAME "wxl-modern-mounts"
#define LOG_CHANNEL "modern-mounts"
#define TABLE_ONE "CreatureDisplayInfo.dbc"
#define TABLE_TWO "CreatureModelData.dbc"
#define TABLE_ROOT "WXL\\ModernMounts\\DBFilesClient\\"
...
static const char* redirect(void* archive, const char* name)
{
    ...
    if (services && services->Log)
        services->Log(WXL_LOG_INFO, LOG_CHANNEL, "creature redirect: %s", replacement);
    return replacement;
}
...
int __cdecl WXL_Load(const WXL_Api* api)
{
    ...
    registered = 1;
    if (services->Log)
        services->Log(WXL_LOG_INFO, LOG_CHANNEL, "locale-independent creature redirects ready");
    return 1;
}
